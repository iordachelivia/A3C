...
def saveToCenter(i,rList,jList,bufferArray,summaryLength,h_size,sess,mainQN,time_per_step):
    with open('./Center/log.csv', 'a') as myfile:
        ...
        make_gif(np.ones([len(imagesS),84,84,3]),'./Center/frames/sal'+str(i)+'.avi',duration=len(imagesS)*time_per_step,true_image=False,salience=True,salIMGS=luminance)

        images = zip(bufferArray[:,0])
        images.append(bufferArray[-1,3])
        images = np.vstack(images)
        images = np.resize(images,[len(images),84,84,3])
        make_gif(images,'./Center/frames/image'+str(i)+'.avi',duration=len(images)*time_per_step,true_image=True,salience=False)

        wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
        wr.writerow([i,np.mean(jList[-100:]),np.mean(rList[-summaryLength:]),'./frames/image'+str(i)+'.avi','./frames/log'+str(i)+'.csv','./frames/sal'+str(i)+'.avi'])
        myfile.close()
...
def make_gif(images, fname, duration=2, true_image=False,salience=False,salIMGS=None):
  ...
  if salience == True:
    ...
    mask.write_videofile(fname, fps = len(images) / duration,verbose=False, codec = 'rawvideo')
    #clipB.write_gif(fname, fps = len(images) / duration,verbose=False)
  else:
    #clip.write_gif(fname, fps = len(images) / duration,verbose=False)
    clip.write_videofile(fname, fps=len(images) / duration, verbose=False,codec = 'rawvideo')
import imageio
